...
/**
 * 合并排序： 特点： 以空间换时间 核心算法： 如何将两个有序数组合并成一个有序数组 实现： 1.拆：将一个大的数组一直拆到n个长度为g的数组 2.排序：
 * 每两个长度为一的数组排序 3.合并：将拍好序的数组合并
 * 
 * @author Eden
 *
 */
public class MergeSorter implements Sortable {
	...
	public void sort() {
		...
		System.arraycopy(a, first.length, second, 0, second.length); // first=>
																		// 3
																		// seond:1
		// 递归再分
	...
	private void merge(int[] first, int[] second) {
		...
		// 预防second数组先运行完
		System.arraycopy(second, iSecond, a, j, second.length - iSecond); // a:
																			// 1,3
	}
	...
	public static void main(String[] args) {
		 ...
		 * 第一小步，实现两个有序数组的合并
		 * 
		 * int[] x={2,66,99}; int[] y={3,6,90};
		 * 
		 * int[] r=new int[6]; MergeSorter ms=new MergeSorter(r); ms.merge(x,
		 * y); for( int t: ms.a){ System.out.print( t +"\t"); }
		 */
